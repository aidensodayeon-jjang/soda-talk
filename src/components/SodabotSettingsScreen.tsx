              ...
              {!generationResult ? (
                ...
                <form id="new-func-form" onSubmit={handleBuildFirmware} className="space-y-4">
                  ...
                  <div className="space-y-3 pt-1 border-t border-[#E5E5E3]">
                    ...
                    {/* GPT 프롬프트 가이드 팝오버 카드 */}
                    {showGptHelp && (
                      <div className="p-3 sm:p-3.5 bg-blue-50/60 border border-blue-200 rounded-2xl space-y-2 animate-fade-in shadow-xs">
                        <div className="flex items-center justify-between">
                          <label className="text-xs font-bold text-[#191919] flex items-center gap-1.5">
                            <span>원하는 기능 입력</span>
                            <span className="text-[10px] text-blue-600 font-normal">
                              {gptFeatureInput.trim() ? '(입력됨)' : funcNameInput.trim() ? `('${funcNameInput}' 자동 반영됨)` : ''}
                            </span>
                          </label>
                          <button
                            type="button"
                            onClick={() => setShowGptHelp(false)}
                            className="w-5 h-5 rounded-md bg-white border border-[#E5E5E3] flex items-center justify-center text-[11px] text-neutral-500 hover:text-neutral-900 cursor-pointer transition-colors shrink-0"
                            title="닫기"
                          >
                            ✕
                          </button>
